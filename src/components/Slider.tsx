  ...
  return <div className="relative w-full h-[calc(100vh-64px)]">
      ...
      <div className="w-full h-full flex justify-center items-center">
        <div className="w-full h-full relative bg-gray-400">
          {slides.map((slide, index) => <Link key={index} to={slide.link} className={`absolute top-0 left-0 w-full h-full ${getSlideClass(index)} transition-all duration-500 ease-in-out group`}>
      ...
      <div className="hidden">
        <SliderControls totalSlides={slides.length} currentIndex={currentIndex} goToSlide={goToSlide} prevSlide={prevSlide} nextSlide={nextSlide} />
      </div>
    </div>;
};
